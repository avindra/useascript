...
`))+"</code></pre>");continue}let c=[];for(;n<e.length&&e[n].trim()&&!/^\s*>/.test(e[n])&&!/^\s{2,}\S/.test(e[n]);)c.push(e[n++]);o.push("<p>"+O(c.join(" "))+"</p>")}return o.join("")}function he(t){if(!t||T.has(t))return;let e=t.querySelector(".commtext");if(!e)return;let o=e.innerText;e.innerHTML=ue(o),T.add(t)}document.querySelectorAll(".athing.comtr").forEach(he)});var _=m(()=>{var ge=`
main {
